...
#### ✅ タスク 15: Cloud Monitoring 設定（低コスト版）

> [!NOTE]
> GCP 無料枠内で実現可能な最小限の監視を構築します。
> カスタムメトリクスや高度な通知連携は将来対応。

- [x] **15-1: 通知チャンネル設定**
  - Email のみ（Slack/PagerDuty は保留）
- [x] **15-2: アラートポリシー設定**
  - エラー率 > 5% (5xx エラー)
  - レスポンス遅延 (p95) > 5 秒
  - **[Quality]** 閾値を変数化し、バリデーションを追加
  - **[Quality]** アラート通知にプレイブック（対応手順）へのガイド記述を追加
- [x] **15-3: ダッシュボード（オプション・手動）**
  - GCP コンソールで手動作成推奨
  - リクエスト数 / エラー率 / レスポンス遅延 / インスタンス数
...
#### ✅ タスク 16: ロギングと分析設定（低コスト版）

> [!NOTE]
> GCP 無料枠内で実現。BigQuery エクスポートは将来対応。

- [x] **16-1: 除外フィルター設定**
  - ヘルスチェック成功ログを除外（ノイズ削減）
  - DEBUG ログを除外（prod のみ）
  - **[Quality]** `disabled` 属性で運用時の一時無効化が可能
  - **[Quality]** `try()` による堅牢な output 実装
  - **[Quality]** 将来拡張候補をコメントで明示
- [x] **16-2: ログ保持期間**
  - デフォルト 30 日を維持（無料）
- [ ] ~~ログベースメトリクス定義~~ → **保留**（タスク 15 でカバー）
- [ ] ~~BigQuery へのエクスポート~~ → **保留**（コスト発生）

#### 🔄 タスク 17: パフォーマンスモニタリング（低コスト版）

> [!NOTE]
> GCP 無料枠内で実現。Terraform 実装は最小限。

- [ ] **17-1: Cloud Trace（トレーシング）**
  - Cloud Run 自動トレースを利用（設定不要、無料）
  - GCP コンソールで確認可能
- [ ] **17-2: Cloud Profiler（プロファイリング）**
  - 完全無料だがアプリケーション側設定が必要
  - → バックエンドドキュメントに手順追記
- [ ] **17-3: Uptime Checks（稼働率監視）**
  - ヘルスチェックエンドポイント監視（100 件/月無料）
  - GCP コンソールで手動設定推奨
- [ ] ~~Cloud Debugger~~ → **削除（2023 年 5 月廃止）**
