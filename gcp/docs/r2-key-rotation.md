...
Secret Manager は新しいバージョンのシークレットを作成します。
**重要**: 実行中の Cloud Run サービスに新しいシークレットを反映させるには、**新しいリビジョンのデプロイ（またはサービスの更新）が必要**です。環境変数はコンテナ起動時に注入されるため、再起動なしには反映されません。

```bash
# 例: 新しいリビジョンを強制的に作成してシークレットを読み込み直す
gcloud run services update pose-est-backend-${ENV} --force-new-revision
```
