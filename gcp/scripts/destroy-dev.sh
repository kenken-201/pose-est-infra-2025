...
# Terraform Destroy 実行スクリプト (Dev 環境)
# -----------------------------------------------------------------------------
# .env から環境変数を読み込み、terraform destroy を実行します。
# 開発環境のリソースを全て削除します。
...
echo "⚠️  警告: 開発環境 (dev) の GCP リソースを全て削除しようとしています！"
echo "  Project: ${GCP_PROJECT_ID:-kenken-pose-est}"
echo "  Target:  gcp/terraform/environments/dev"
echo ""
...
echo "🔥 Terraform Destroy を実行中 (Dev)..."
# ユーザー確認を求めるため -auto-approve は付けません
terraform destroy
