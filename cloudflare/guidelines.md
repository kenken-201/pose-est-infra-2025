...
graph LR
    ...
    B --> C[Cloudflare Workers<br/>フロントエンドビルド&デプロイ]
    B --> D[Cloudflare API経由<br/>DNS/WAF/R2設定更新]
    C --> E[Cloudflareグローバルエッジ]
    D --> F[R2ストレージバケット設定]
    E --> G[ユーザーアクセス]
    F --> H[バックエンド署名URL生成]
    H --> I[クライアント直接R2アクセス]
...
| 技術      | バージョン | 用途                                                                  |
| --------- | ---------- | --------------------------------------------------------------------- |
| Terraform | 1.14+      | Cloudflare リソースのプロビジョニング### コーディング規約 (Terraform) |

#### コメントルール

可読性を高めるため、以下のルールに従ってコメントを記述してください。

- **`/* */` (複数ブロックの注釈)**
  - 複数のリソースやブロックを意味的にまとめる場合、その塊の最初のブロックの上に記述します。

- **`#` (複数行の注釈)**
  - 複数行にわたる設定やロジックに対する注釈が必要な場合、該当コードの上に記述します。
  - 可読性を確保するため、対象コードの前後に空行を入れます。

- **`#` (単一行の注釈)**
  - 単一行のコードに対する注釈は、原則として該当コードの右側に記述します。
  - ただし、コードが長くなる場合や説明が長い場合はコードの上に記述します。

- **`//` (一時的なコメントアウト)**
  - デバッグなどで一時的にコードを無効化する場合のみ使用します。コミットするコードには残さないようにします。

#### 言語

- コメントは原則として **日本語** で記述してください。生成 |
